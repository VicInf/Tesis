<?php
    ...
    $levelQuery = "SELECT reachindex, level FROM usuarios WHERE username='" . $playerToken . "';";
    ...
    {
        $reachedIndex = $row['reachindex'];
        $level = $row['level'];

        // Check if the last scene played is greater than or equal to the ReachedIndex
        if ($lastScenePlayed >= $reachedIndex) 
        {
            // If so, increment both ReachedIndex and level
            $reachedIndex = $lastScenePlayed + 1;
            $level = $level + 1;
            
            // Save the new values back to the database
            $updateQuery = "UPDATE usuarios SET reachindex = " . $reachedIndex . ", level = " . $level . " WHERE username = '" . $playerToken . "';";
